...
interface MentalHealthNursingReport {
  patientName: string;
  birthDate: string;
...
@Component({
  selector: 'app-mental-health-nursing-report-form',
  templateUrl: './mental-health-nursing-report-form.component.html',
  styleUrls: ['./mental-health-nursing-report-form.component.css'],
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule]
})
export class MentalHealthNursingReportFormComponent implements OnInit {
  ...
  createForm(): FormGroup {
    ...
    const form = this.fb.group({
      patientName: ['鈴木 一郎', Validators.required],
      birthYear: ['1960', Validators.required],
      birthMonth: ['5', Validators.required],
      birthDay: ['10', Validators.required],
      age: [''],
      caregiverFacility: ['精神科訪問看護ステーション'],
      requireCareLevel: [5], // 要介護3
      visitDatesPrevMonth: this.createVisitDatesArray(),
      visitDatesCurrentMonth: this.createVisitDatesArray(),
      specialNotes: ['安定した状態を維持している'],
      nursingContent: ['バイタルサイン測定、服薬管理、日常生活指導、精神状態の評価'],
      familyCooperation: ['家族の協力は良好。妻が日常生活のサポートを行っている'],
      materialName: [''],
      usagePurpose: [''],
      usageAmount: [''],
      ...
      reportMonth: ['3'],
      reportDay: ['29'],
      recipientName: ['精神 太郎'], // 宛先の名前（殿の前に表示）
      managerName: ['']
    });
  ...
  generatePDF(): void {
    const reportElement = document.getElementById('mental-health-nursing-report');
    if (!reportElement) {
